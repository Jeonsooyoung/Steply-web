  ...
  return (
    <PageShell active="Assessment" className="ref-assessment-page">
      <main className="ref-page ref-assessment">
        <ScreenHeading title="Assessment Center" subtitle="Connect your camera, complete the guided balance test, and review your latest results." />

        <section className="ref-assessment-banner">
          <div><span>READY FOR TODAY</span><h2>Your balance check is ready.</h2><p>Choose a step below. You can pause at any time and continue when you feel comfortable.</p></div>
          ...
          <section className="ref-assessment-steps">
            {assessmentSteps.map((step, index) => (
              <Panel className="ref-assessment-step" key={step.number}>
                ...
                <div><span>{index === 0 ? 'SETUP' : index === 1 ? 'ASSESSMENT' : 'RESULTS'}</span><h2>{step.title}</h2><p>{step.description}</p></div>
